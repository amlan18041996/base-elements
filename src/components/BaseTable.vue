  ...
  <div :class="['table-container', overrideStyle]">
    <div class="controls">
      ...
      <div class="flex gap-x-4">
        <base-dropdown
          name="Filters"
          toggler-class="btn-sm btn-outline-primary rounded-sm"
          display-class="flex-col p-4 bg-white rounded border shadow right-0"
        >
          <form :onsubmit="(e) => e.preventDefault()" class="space-y-2">
            <template
              v-for="(headerObj, headerName, index) in headers"
              :key="index"
            >
              <template
                v-if="
                  headerObj.filter.search !== undefined &&
                  ['text', 'number'].includes(headerObj.type)
                "
              >
                <base-input
                  ...
                  :base-type="headerObj.type"
                ></base-input>
              </template>
              <template
                v-if="
                  headerObj.filter.search !== undefined &&
                  ['date'].includes(headerObj.type)
                "
              >
                <div class="p-1.5 bg-gray-50 space-y-1">
                  <!-- Implement Calendar -->
                </div>
              </template>
            </template>
            <div class="flex gap-3">
              <button
                class="btn btn-sm btn-warning w-full font-semibold tracking-widest"
                @click="filterSearched"
              >
                APPLY
              </button>
              <button
                class="btn btn-sm btn-outline-warning w-full font-semibold tracking-widest"
                @click="resetSearched"
              >
                RESET
              </button>
        ...
        <button
          class="btn btn-sm btn-primary rounded-sm space-x-2"
          v-if="downloadable"
        >
          <span>Download</span>
          <svg
    ...
    <div class="wrapper">
      <table :class="['table', tableStyle]">
        <thead v-if="!disableHeading">
          <tr>
            <template
              v-for="(head, keyName, headIndex) in headers"
              :key="headIndex"
            >
              <th scope="col" :class="head.overideClass" v-if="head.show">
                <div class="head-wrapper flex items-center gap-2">
                  ...
                  <span
                    ...
                    @click="
                      applyFilters('sort', {
                        keyName,
                        sort: ['', 'dsc'].includes(head.filter.sort)
                          ? 'asc'
                          : 'dsc',
                      })
                    "
                  >
                    <slot
                      name="headIcon"
                      :filter="{ key: keyName, ...head.filter }"
                    >
                      <template v-if="head.type === 'text'">
                        <svg
                          v-if="['', 'dsc'].includes(head.filter.sort)"
                          xmlns="http://www.w3.org/2000/svg"
                          width="16"
                          height="16"
                          fill="currentColor"
              ...
              >
                <td v-if="head.show">
                  ...
                  <template v-else>
                    <div
                      class=""
                      v-if="head.type === 'date' && head.parse && head.format"
                    >
                      {{ useDateFormat(item[keyName], head.format).value }}
                    </div>
                    <div class="" v-else>{{ item[keyName] }}</div>
                  </template>
                </td>
              </template>
            </TransitionGroup>
    ...
    <div id="table-navigation" v-if="lists.total > lists.perPage">
      <div class="showing">
        ...
        <span class="hightlight">
          ...
          to
          {{
            lists.currentPageNumber * lists.perPage > lists.total
              ? lists.total
              : lists.currentPageNumber * lists.perPage
          }}
        </span>
        out of <span class="hightlight">{{ lists.total }} entries</span>
      </div>
      <div :class="['table-pagination', paginateDottedStyle]">
        ...
        <template v-else-if="paginateLinks === 'dotted'">
          <template :key="pageNum" v-for="pageNum in lists.dottedPages">
            <button
              :class="[
                { active: pageNum === changedPageNumber },
                { 'never-active': pageNum === '...' },
              ]"
        ...
        <button
          @click="changePage(changedPageNumber + 1)"
          :disabled="
            changedPageNumber >= Math.ceil(detectList.length / lists.perPage)
          "
        >
          Next
        </button>
...
function changePage(num) {
  const items =
    Object.keys(searches).length > 0 && lists.filtered.length > 0
      ? lists.filtered
      : lists.all;
...
function filterSearched() {
  Object.keys(headers.value).forEach((head) => {
    if (headers.value[head].filter.search !== undefined) {
      if (head in searches.value && headers.value[head].filter.search === "")
        delete searches.value[head];
      else if (headers.value[head].filter.search !== "")
        searches.value = {
          ...searches.value,
          [head]: headers.value[head].filter.search,
        };
    }
  });
  evaluateSearch();
...
function resetSearched() {
  Object.keys(headers.value).forEach((head) => {
    if (headers.value[head].filter.search !== undefined)
      headers.value[head].filter.search = "";
...
function evaluateSearch() {
  ...
  if (Object.keys(searches.value).length > 0) {
    listClone = listClone.filter((list) => {
      ...
      let everyFilter = Object.keys(searches.value).every((val) => {
        if (
          typeof list[val] !== "undefined" &&
          typeof headers.value[val].filter.search !== "undefined"
        ) {
          if (["text", "number"].includes(headers.value[val].type)) {
            if (
              String(list[val])
                .toLowerCase()
                .includes(String(searches.value[val]).toLowerCase())
            ) {
            ...
            if (
              ...
              parseInt(Date.parse(searches.value[val].dateFrom)) <=
                parseInt(list[val]) &&
              searches.value[val].dateTo &&
              parseInt(Date.parse(searches.value[val].dateTo)) >=
                parseInt(list[val])
            ) {
              approvedCount = approvedCount + 1;
              return true;
          ...
          let orResult = specialSearch.filter((sp) =>
            String(list[sp])
              .toLowerCase()
              .includes(String(searches.value[val].value).toLowerCase())
          );
      ...
      return approvedCount >= Object.keys(searches.value).length
        ? everyFilter
        : false;
    });
  }
  ...
  const activePageNumber =
    Math.ceil(results.length / lists.perPage) === lists.currentPageNumber
      ? lists.currentPageNumber
      : 1;
  lists.filtered = listClone;
  lists.total = listClone.length;
  changedPageNumber.value = activePageNumber;
  const { dotted, striped } = paginateList(
    results,
    activePageNumber,
    lists.perPage
  );
...
function evaluateSort() {
  ...
  const activeSort = Object.keys(headers.value).find(
    (filter) => headers.value[filter].active
  );
  if (
    ...
    listClone = listClone.sort((x, y) => {
      if (["text"].includes(headers.value[activeSort].type))
        return headers.value[activeSort].filter.sort === "asc"
          ? x[activeSort].localeCompare(y[activeSort])
          : y[activeSort].localeCompare(x[activeSort]);
      if (["number", "date"].includes(headers.value[activeSort].type))
        return headers.value[activeSort].filter.sort === "asc"
          ? parseInt(x[activeSort]) - parseInt(y[activeSort])
          : parseInt(y[activeSort]) - parseInt(x[activeSort]);
    });
  }
  const { dotted, striped } = paginateList(
    listClone,
...
const detectList = computed(() =>
  lists.filtered.length > 0 ? lists.filtered : lists.all
);

// Initial Phase
onMounted(() => {
  if (props.response && Object.keys(props.response).length > 0) {
    ...
    Object.keys(headings).map((head) => {
      lists.all = items.map((item) => {
        if (
          head === "date" &&
          headings[head].parse &&
          !isNaN(Date.parse(item[head]))
        ) {
          item[head] = Date.parse(item[head]);
        }
        return item;
      });
    });
