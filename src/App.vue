  ...
  <div class="container mx-auto my-4 space-y-5 px-4">
    <div class="grid grid-cols-9 border grid-flow-col">
      <div class="hidden md:block col-span-2 card gap-3">
        <div class="card-header text-center font-semibold tracking-wider">
          Select PAGE
        </div>
        ...
        <div class="card-body" id="bod">
          <template v-if="show === 'table'">
            ...
            >
              ...
              ></base-input>
              <button
                class="btn btn-sm btn-outline-primary font-semibold"
                @click="service.execute"
              >
                Fetch User Details
              </button>
            </div>
            ...
            >
              <template #load>
                <div
                  class="w-full p-4 text-xl text-blue-500 text-center font-semibold tracking-wider rounded shadow"
                >
                  Please wait, retrieving user details...
                </div>
              </template>
          ...
          <template v-if="show === 'modal'">
            ...
            <base-modal :show="modal" display-head display-footer port="bod">
              ...
              <template #body> Welcome to base-components package </template>
              <template #footer>
                <button class="btn btn-primary" @click="modal = false">
                  Okay
                </button>
              </template>
            </base-modal>
          ...
          <template v-if="show === 'dropdown'">
            <div class="flex gap-5">
              <base-dropdown
                ...
                display-class="w-64 flex-col p-4 bg-white rounded border shadow"
              >
              ...
              <base-calendar v-model="dateFrom.data"></base-calendar>
            </div>
          </template>
          ...
          <template v-if="show === 'toggle'">
            <div class="flex justify-between">
              ...
              >
                <template #signs>
                  <svg
                    v-if="emailPrefer && !emailPrefer.active"
...
const show = ref("dropdown");
const modal = ref(false);
const emailPrefer = ref(null);
const dateFrom = ref({ data: null });
const params = ref("?id=7237&name=bob&age=33");
const url = ref(
  "https://971dbe15-5a27-4e6c-9e76-bdeafe344147.mock.pstmn.io/v2/order-list"
);
const allMethods = [
